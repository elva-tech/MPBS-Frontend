...
import { getShipmentStatusLabel, fetchShipments, patchShipment, setActiveShipmentId } from "./state";
...
export default function DairyRouteSheets() {
  ...
  const handleStartVerification = async () => {
    if (nextShipmentId) {
      const pending = shipments.find((item) => item.id === nextShipmentId);
      if (pending?.status === "pending") {
        await patchShipment(nextShipmentId, { status: "in_verification" });
      }
      ...
      <div className="mt-5 overflow-hidden rounded-xl border border-[#D7E4FF] bg-white shadow-[0_4px_12px_rgba(15,41,74,0.08)]">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[820px] text-left text-sm">
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={7} className="px-4 py-6 text-center text-[#5B6B7F]">Loading route sheets...</td>
                </tr>
              ) : routeSheetRows.length === 0 ? (
                <tr>
                  <td colSpan={7} className="px-4 py-6 text-center text-[#5B6B7F]">No tanker shipments for today.</td>
                </tr>
              ) : (
        ...
        <div className="flex items-center justify-between border-t border-[#E6EDF7] bg-[#FAFCFF] px-4 py-3">
          ...
          <button
            ...
            disabled={!nextShipmentId || loading}
            className="rounded bg-[#1E4B6B] px-4 py-2 text-sm font-semibold text-white hover:bg-[#173A55] disabled:opacity-50"
          >
            Start Verification
          </button>
        </div>
